...
function example2(response, request) {
    //Include this line for memcache
    ...
    response.setHeader("Content-Type", "text/html");
    WurflCloudClientObject.detectDevice(request, result_capabilities, function(err, result_capabilities){
        console.log("All capabilities available:" + JSON.stringify(result_capabilities));
        if(err!=null){
            response.write("<br>Error: " + err + " <br/>");
        }else{
            response.write("<br>"+ 'Brand Name: ' + result_capabilities['complete_device_name'] + " <br/>");
            response.write("<br>"+ 'Is Wireless Device: ' + result_capabilities['is_mobile'] + " <br/>");
        }
        response.end();
    });
}
